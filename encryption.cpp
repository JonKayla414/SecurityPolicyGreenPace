...
//
#define _CRT_SECURE_NO_WARNINGS // use this so vs doesntt flag
...
{
    std::string file_text = "Jon Kayla Pointer \nSNHU CS 405";
...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
    ...
    char buf[100] = { 0 };
    strftime(buf, sizeof(buf), "%Y-%M-%D", localtime(&timeNow));

    ofstream file_stream(filename);
    file_stream << student_name << endl;
    file_stream << buf << endl;
    file_stream << key << endl;
    file_stream << data << endl;
    file_stream.close();
